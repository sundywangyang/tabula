...
/**
 * Windows: 用 PowerShell Get-PSDrive 拉所有 FileSystem provider 的盘符。
 * 失败/非 Windows 降级为 statfsSync(只列当前盘)+ 至少返回 C: 一个固定项。
 */
export async function listDrives(): Promise<DriveInfo[]> {
  if (process.platform === 'win32') {
    try {
      ...
      if (drives.length === 0) return fallbackDrives();
      return drives;
    } catch (err) {
      console.warn('[fs] listDrives PowerShell failed, fallback:', (err as Error).message);
      return fallbackDrives();
    }
  }
  // POSIX: statfs /、常用 mount 点
  try {
    const info = statfsSync('/');
    return [
      {
        mount: '/',
        label: '根分区',
